...
| Queue semantics (consumer groups, acks, redelivery) | 🎯 Target | Not usable. Durable group cursors exist — a group's position on a shard is recorded as a key → latest-value projection over a log, monotonic so a late acknowledgement cannot rewind it, and it survives a restart. The delivery rules above the cursor are built too — a claimed record is not handed to a second consumer, a lapsed claim becomes owed again, and the cursor advances only over a contiguous run of acknowledgements. Nothing is reachable: there is no wire protocol, so no client can subscribe as a group, acknowledge, or be redelivered to (#280) |
