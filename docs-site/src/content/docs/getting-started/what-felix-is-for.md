...
| Raft consensus for cluster metadata | 🎯 Target | Designed, not implemented: [`docs/metadata-raft-design.md`](https://github.com/gabloe/felix/blob/main/docs/metadata-raft-design.md) is the decided design (openraft group inside the control-plane instances, the in-memory store as the state machine, the broker contract frozen), tracked as milestone M13 under [#333](https://github.com/gabloe/felix/issues/333). Until it lands, control-plane metadata availability rests on HA Postgres per [`docs/ha-postgres.md`](https://github.com/gabloe/felix/blob/main/docs/ha-postgres.md). Raft here is for *metadata* only, deliberately not for replicating stream records: shard replication is leader leases plus log shipping, decided in [`docs/replication-design.md`](https://github.com/gabloe/felix/blob/main/docs/replication-design.md) |
