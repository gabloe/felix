...
| Raft consensus for cluster metadata | 🚧 Partial | The consensus core and the metadata state machine are merged: an openraft group embedded in the control-plane instances behind a seam, a crash-safe log/vote/snapshot store passing openraft's own storage conformance suite, and the metadata store driven by a versioned API-shaped command set — held to byte-identical determinism by a harness, with concurrent tenant bootstraps settled by log order alone on a real three-node group. Partial because **the control-plane API does not serve from it yet**: the store backend, probes/packaging, and the Postgres migration are the rest of milestone M13 ([#333](https://github.com/gabloe/felix/issues/333)); until they land, control-plane metadata availability rests on HA Postgres per [`docs/ha-postgres.md`](https://github.com/gabloe/felix/blob/main/docs/ha-postgres.md). Design: [`docs/metadata-raft-design.md`](https://github.com/gabloe/felix/blob/main/docs/metadata-raft-design.md). Raft here is for *metadata* only, deliberately not for replicating stream records: shard replication is leader leases plus log shipping, decided in [`docs/replication-design.md`](https://github.com/gabloe/felix/blob/main/docs/replication-design.md) |
