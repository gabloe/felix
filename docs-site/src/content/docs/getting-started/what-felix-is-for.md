...
| Non-Rust client SDKs | 🚧 Partial | Python ships, as a binding over the Rust client rather than a reimplementation, and passes every required scenario in the client conformance catalogue. Consumer groups, cache watches and multi-shard subscribe are not wrapped yet, and the catalogue records them as unclaimed rather than passing. TypeScript, Go and C# are not started; each will be gated on the same suite. See [Clients in Other Languages](/felix/api/clients/) |
