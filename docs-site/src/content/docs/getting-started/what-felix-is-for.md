...
| Cross-region routing and data sovereignty enforcement | 🎯 Target | Not implemented. `felix-router` defines a directional region-pair allowlist, but nothing in the request path calls it: no publish, subscribe or replica placement is ever refused for crossing regions |
...
are injected across the network, the process and the control plane, but not yet
at the disk, so "a broker must not acknowledge what the device refused" is the
one safety property nothing exercises
([#135](https://github.com/gabloe/felix/issues/135)), and there is no
cluster-scale latency budget
