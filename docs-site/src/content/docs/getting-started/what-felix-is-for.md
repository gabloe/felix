...
| Raft consensus for cluster metadata | 🚧 Partial | The consensus core is merged: an openraft group embedded in the control-plane instances, behind a seam, with a crash-safe log/vote/snapshot store that passes openraft's own storage conformance suite — proven by group tests for election, replication, restart-as-rejoin, wiped-volume rebuild by snapshot, and learner-first growth. Partial because **no metadata rides it yet**: the state machine, the store backend, and the Postgres migration are the rest of milestone M13 ([#333](https://github.com/gabloe/felix/issues/333)); until they land, control-plane metadata availability rests on HA Postgres per [`docs/ha-postgres.md`](https://github.com/gabloe/felix/blob/main/docs/ha-postgres.md). Design: [`docs/metadata-raft-design.md`](https://github.com/gabloe/felix/blob/main/docs/metadata-raft-design.md). Raft here is for *metadata* only, deliberately not for replicating stream records: shard replication is leader leases plus log shipping, decided in [`docs/replication-design.md`](https://github.com/gabloe/felix/blob/main/docs/replication-design.md) |
