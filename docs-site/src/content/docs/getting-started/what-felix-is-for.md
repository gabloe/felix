...
| Raft consensus for cluster metadata | 🎯 Target | Not started; tracked in [#333](https://github.com/gabloe/felix/issues/333). Until then control-plane metadata availability rests on HA Postgres per [`docs/ha-postgres.md`](https://github.com/gabloe/felix/blob/main/docs/ha-postgres.md). Raft is the intended mechanism for making control-plane *metadata* highly available, and deliberately not for replicating stream records: shard replication is leader leases plus log shipping, decided in [`docs/replication-design.md`](https://github.com/gabloe/felix/blob/main/docs/replication-design.md) |
