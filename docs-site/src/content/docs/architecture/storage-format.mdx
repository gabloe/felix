---
title: "Felix Durable Segment Format (v1)"
description: "The versioned on-disk byte layout for Felix durable stream segments and indexes."
---

import segmentFile from '../../../../../docs/assets/storage/segment-file.svg';
import segmentHeader from '../../../../../docs/assets/storage/segment-header.svg';
import record from '../../../../../docs/assets/storage/record.svg';
import indexHeader from '../../../../../docs/assets/storage/index-header.svg';
import indexEntry from '../../../../../docs/assets/storage/index-entry.svg';
...
Implementation:
[`crates/server/felix-storage/src/segment/format.rs`](https://github.com/gabloe/felix/blob/main/crates/server/felix-storage/src/segment/format.rs).
The two must move together — a change to one without the other is a bug.

This page is the byte layout. For how those bytes are produced and consumed over
time — appending, the sparse index, sealing, rollover and retention — see
[Durable Storage](/felix/architecture/durable-storage/#on-disk-layout), which
walks both through animated diagrams.
...
<p align="center">
  <img src={segmentFile.src} alt="A segment file: a 32-byte header followed by variable-length records, with a sparse index file whose entries point at record boundaries" width="900" />
</p>

### Segment header (32 bytes)

<p align="center">
  <img src={segmentHeader.src} alt="Segment header byte layout: magic, version, flags, base_offset, created_at_micros, header_crc, reserved" width="882" />
</p>
...
<p align="center">
  <img src={record.src} alt="Record byte layout: payload_len, offset, timestamp_micros, checksum, then the payload. The checksum covers bytes 0 to 20 and the payload" width="882" />
</p>
...
<p align="center">
  <img src={indexHeader.src} alt="Index header byte layout: magic, version, flags, base_offset, reserved" width="672" />
</p>
...
<p align="center">
  <img src={indexEntry.src} alt="Index entry byte layout: an eight-byte logical offset and an eight-byte file position" width="672" />
</p>
...
[`segment/reader.rs`](https://github.com/gabloe/felix/blob/main/crates/server/felix-storage/src/segment/reader.rs).
