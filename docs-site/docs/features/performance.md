...
**Expected performance**: see [Benchmarks](benchmarks.md) for current, measured
...
[Benchmarks](benchmarks.md) (batch = 1, per-message acked) measures this
...
[Benchmarks](benchmarks.md) (batch = 64, lossless, zero drops) measures this
...
!!! note "Lossless pacing is an explicit trade-off"
    ...
    [Benchmarks: Saturation behavior](benchmarks.md#saturation-behavior).
...
!!! note "What lanes parallelize changed"
    ...
    [Wire Protocol: Shared Binary EventBatch](../architecture/wire-protocol.md#shared-binary-eventbatch-encoding)).
    ...
    *write* syscalls across subscribers. Older lane-count sweep numbers from
    ...
    been removed; see [Benchmarks](benchmarks.md) for current measurements.
...
5. For multi-stream workloads, also evaluate `core_shards` (thread-per-core
   stream ownership) — see [Benchmarks](benchmarks.md), which showed larger
...
See **[Benchmarks](benchmarks.md)** for current, methodology-documented
...
!!! note
    ...
    [Benchmarks](benchmarks.md) — the cache path uses the same QUIC
...
5. Check path MTU discovery (`FELIX_MTU_UPPER_BOUND`) - see [Benchmarks](benchmarks.md) for why this matters more than it looks.
...
!!! note "These bands are illustrative, not measured"
    Single-broker measurements in [Benchmarks](benchmarks.md) reach into the
