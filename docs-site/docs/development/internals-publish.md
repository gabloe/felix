...
[Client SDK](../api/client-sdk.md) for that).
...
2. **Encoding.** If `ack == AckMode::None`, the message is binary-encoded by
   ...
   [Wire Protocol](../architecture/wire-protocol.md#binary-publish-batch-encoding).
...
3. **Admission.** Before the message is handed to the worker's channel, the
   ...
   [Internals: Backpressure](internals-concurrency.md) for why that timing
   matters.
...
2. **Admission.** Mirrors the client exactly: `enqueue_publish()` computes
   ...
   see [Internals: Backpressure](internals-concurrency.md)). This is where
...
3. **`DeliveryEnvelope::new`**: wraps `payloads` in one `Arc<[Bytes]>` plus a
   ...
   see [Internals: Subscribe & Fanout](internals-subscribe.md)). Every
...
4. **Per-subscriber send**, gated by `stream_state.subscriber_queue_policy`
   ...
   [Internals: Backpressure](internals-concurrency.md#the-full-backpressure-chain)
...
6. Each subscriber's feeder task independently calls
   ...
   cached `Bytes` for free. See [Internals: Subscribe & Fanout](internals-subscribe.md).
