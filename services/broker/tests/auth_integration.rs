...
use felix_authz::{
    Action, FelixClaims, FelixTokenIssuer, Jwk, Jwks, KeyUse, TenantId, TenantKeyMaterial,
};
...
fn encode_claims(claims: &FelixClaims, kid: &str, pem: &str) -> String {
...
async fn broker_auth_rejects_invalid_claims_and_signature() -> Result<()> {
    ...
    let wrong_aud_token = encode_claims(&wrong_aud, "k1", TEST_PRIVATE_KEY_PEM);
    ...
    let expired_token = encode_claims(&expired, "k1", TEST_PRIVATE_KEY_PEM);
