...
use felix_controlplane_service::{config, membership, observability, placement, store};
...
async fn main() -> anyhow::Result<()> {
    ...
    if args.first().map(String::as_str) == Some("migrate") {
        args.remove(0);
        return felix_controlplane_service::migrate::run(args).await;
    }
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    let bootstrap_task = if config.bootstrap.enabled {
        ...
        let bootstrap_tls = config
            ...
            .tls
            .as_ref()
            .map(felix_controlplane_service::tls::load_server_config)
            .transpose()?;
        Some(tokio::spawn(async move {
            ...
            match tokio::net::TcpListener::bind(bootstrap_addr).await {
                Ok(listener) => match bootstrap_tls {
                    Some(tls) => {
                        felix_controlplane_service::tls::serve_mtls(
                            listener,
                            bootstrap_app,
                            tls,
                            api_shutdown,
    ...
    let store: Arc<dyn ControlPlaneAuthStore + Send + Sync> = match config.storage {
        ...
        config::StorageBackend::Raft => {
            ...
            let handle = RaftHandle::start(
                settings,
                Arc::clone(&machine) as Arc<dyn felix_controlplane_service::raft::AppStateMachine>,
            )
            .await?;
