...
impl ControlPlaneStore for PostgresStore {
    ...
    async fn register_node(&self, node: Node) -> StoreResult<Node> {
        ...
        .await?;
        tx.commit().await?;
        crate::cluster::membership::metrics::record_registration(
            if stored.status.incarnation == 0 {
                "new"
            } else {
                "restart"
            },
...
impl AuthStore for PostgresStore {
    ...
    async fn take_refresh_token(
        ...
        if let Some(row) = updated {
            return Ok(RefreshTokenTake::Taken(Box::new(Self::refresh_row(
                tenant_id, token_id, &row, true, false,
            )?)));
        }
        ...
        if used {
            return Ok(RefreshTokenTake::Replayed(Box::new(Self::refresh_row(
                tenant_id, token_id, &row, used, revoked,
            )?)));
        }
