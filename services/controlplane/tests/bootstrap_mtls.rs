...
use controlplane::config::BootstrapTlsConfig;
use controlplane::store::{ControlPlaneAuthStore, ControlPlaneStore, StoreConfig};
...
fn state_with_token() -> AppState {
    ...
    AppState {
        ...
        readiness: Arc::new(controlplane::readiness::Readiness::new(Arc::new(
...
async fn serve(
    ...
    let tls = controlplane::tls::load_server_config(&BootstrapTlsConfig {
    ...
    let task = tokio::spawn(controlplane::tls::serve_mtls(
        listener,
        router,
        tls,
        shutdown.clone(),
...
fn missing_key_material_fails_config_load() {
    let err = controlplane::tls::load_server_config(&BootstrapTlsConfig {
