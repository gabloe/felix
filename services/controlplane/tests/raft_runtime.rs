...
fn http(addr: SocketAddr, method: &str, path: &str, body: Option<&str>) -> Option<(u16, String)> {
...
fn wait_ready(child: &mut std::process::Child, addr: SocketAddr) {
    ...
    loop {
        if let Some((200, _)) = http(addr, "GET", "/v1/system/ready", None) {
            return;
        }
...
fn the_binary_serves_and_survives_a_restart_with_no_database() {
    ...
    let (status, _) = http(
        ...
        "POST",
        "/v1/tenants",
        Some(r#"{"tenant_id": "t1", "display_name": "Tenant One"}"#),
    )
    ...
    let (status, body) = http(addr, "GET", "/v1/tenants", None).expect("list tenants");
