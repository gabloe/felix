...
pub(crate) async fn handle_cache_watch_message(
    ...
    let filter = match (&request.key, &request.prefix) {
        ...
        _ => {
            responder
                .send(Message::error(
                    "cache_watch takes exactly one of key or prefix",
                ))
                .await?;
            return Ok(true);
        }
    ...
    {
        responder
            .send(Message::error("max subscriptions per connection exceeded"))
            .await?;
        return Ok(true);
    }
    ...
    let Some(log) = broker
        ...
        responder
            .send(Message::error("the cache shard's log could not be opened"))
            .await?;
        return Ok(true);
    };
    let tail = match log.tail_offset().await {
        ...
        Err(err) => {
            ...
            responder
                .send(Message::error(format!(
                    "the cache shard's log could not be read: {err}"
                )))
                .await?;
            return Ok(true);
        }
    ...
    let catch_up = match (request.retained, request.from_offset) {
        (true, Some(_)) => {
            ...
            responder
                .send(Message::error(
                    "cache_watch takes retained or from_offset, not both",
                ))
                .await?;
            return Ok(true);
        }
        ...
        CatchUp::Snapshot => {
            match snapshot_entries(broker.cache(), &request, shard, &filter, tail).await {
                ...
                Err(err) => {
                    subscriptions.release();
                    responder
                        .send(Message::error(format!("cache snapshot failed: {err}")))
                        .await?;
                    return Ok(true);
                }
            }
    ...
    let mut event_send = match connection.open_uni().await {
        ...
        Err(err) => {
            subscriptions.release();
            responder.send(Message::error(err.to_string())).await?;
            return Ok(true);
        }
    };
