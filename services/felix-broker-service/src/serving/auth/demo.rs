...
/// A broker verifier and a token it accepts, both for the same tenant.
pub struct DemoAuth {
...
/// Mint a token for `tenant_id` with full stream and cache permissions, and a
/// [`BrokerAuth`] that trusts the key it was signed with.
