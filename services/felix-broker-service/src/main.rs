...
//!   (`felix_broker_service::quic::serve`).
...
use felix_broker_service::credential;
use felix_broker_service::membership;
use felix_broker_service::peer;
use felix_broker_service::replication;
use felix_broker_service::{auth::BrokerAuth, config, durable_config::DurableStorageConfig, quic};
use felix_broker_service::{shard_lifecycle, shard_routing, shard_watch};
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    // client may connect.
    let client_endpoints = Arc::new(felix_broker_service::client_endpoints::ClientEndpoints::new());
...
fn peer_lease_state() -> felix_broker_service::lease::LeaseState {
