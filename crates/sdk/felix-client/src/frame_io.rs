//! Reading and writing `felix-wire` frames on QUIC streams.
//!
//! Every read enforces the configured `max_frame_bytes` before allocating, so
//! a peer cannot make the client reserve an arbitrary buffer.
...
pub(crate) async fn read_frame_into_with_limit(
    ...
    #[cfg(feature = "telemetry")]
    let sample = record_read_await && crate::telemetry::t_should_sample();
    #[cfg(feature = "telemetry")]
    let header_start = crate::telemetry::t_now_if(sample);
    ...
    #[cfg(feature = "telemetry")]
    let payload_start = crate::telemetry::t_now_if(sample);
...
pub(crate) async fn read_frame_cache_timed_into_with_limit(
    ...
    #[cfg(feature = "telemetry")]
    let drain_start = crate::telemetry::t_now_if(sample);
    scratch.clear();
    scratch.resize(length, 0u8);
    recv.read_exact(&mut scratch[..])
        .await
