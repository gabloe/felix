//! Read-only Kafka wire compatibility for a Felix broker.
//!
...
//! Topics are `<namespace>.<stream>`, a partition is a shard, and offsets are
//! Felix's own. Consumer groups are refused with an error the client prints,
//! and nothing can be produced yet. `docs/kafka-compatibility.md` has the
//! whole mapping and the reasons behind it.
//!
